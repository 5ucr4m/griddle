...
async function addVotes(picture_id) {
  console.log("Votando ", dados);
  const url = `/votes`;
  const dados = { user_id, picture_id, kind: "1" };
  //   console.log("addVotes ", dados);
  try {
    const { data } = await api.post(url, dados);
    // console.log("addVotes ", data);
    return data;
  } catch (err) {
    // console.log("addVotesError: ", err.response);
  }
  return data;
}
