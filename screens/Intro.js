...
class Intro extends React.Component {
  _onDone = () => {
    db.transaction(tx => {
      tx.executeSql("SELECT * FROM user_access;", [], (tx, results) => {
        var temp = [];
        if (results.rows.length == 0) {
          // console.log('Showing the intro first time', this.props.navigation.getParam('user'));

          tx.executeSql(
            "INSERT INTO user_access (flags) values ( ?);",
            [1],
            (tx, results) => {
              // console.log(results);
              this.props.navigation.navigate("Home", { TakeSelfie: true });
            },
            (tx, error) => {
